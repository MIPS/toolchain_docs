...
<!-- #AND Adding variables -->
	...
	<xsl:variable name="currentDate"
		select="format-dateTime(current-dateTime(), '[D01]/[M01]/[Y0001]')"/>
	...
	<!-- #AND get book main and secondary titles -->
	
	<xsl:variable name="BookMain">
		<xsl:choose>
	...
	<xsl:variable name="BookSecondary">
		<xsl:choose>
		...
		<xsl:choose>
		    <xsl:when test="$map//*[contains(@class, ' bookmap/organization ')] [1]">
			    <xsl:value-of select="$map//*[contains(@class, ' bookmap/organization ')] [1]"/> 
			       
		     </xsl:when>
		    <xsl:otherwise>
			<xsl:call-template name="getVariable">
			 <xsl:with-param name="id" select="'Company Name'"/>
			</xsl:call-template>
		    </xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
...
<xsl:with-param name="msgnum">004</xsl:with-param>
  ...
  <!-- #AND added this, hopefully to 'define' the booktitlealt -->
  <xsl:template match="/" mode="dita-ot:title-metadata" as="xs:string?">
    <xsl:choose>
      <xsl:when test="exists($map/*[contains(@class, ' bookmap/booktitlealt ')]/*[contains(@class,' bookmap/booktitlealt ')])">
        <xsl:value-of>
          <xsl:apply-templates select="$map/*[contains(@class, ' bookmap/booktitlealt ')]/*[contains(@class,' bookmap/booktitlealt ')][1]" mode="dita-ot:text-only"/>
        </xsl:value-of>
      </xsl:when>
      <xsl:when test="exists($map/*[contains(@class,' topic/booktitlealt ')])">
        <xsl:value-of>
          <xsl:apply-templates select="$map/*[contains(@class,' topic/booktitlealt ')][1]" mode="dita-ot:text-only"/>
        </xsl:value-of>
      </xsl:when>
      <xsl:when test="exists(//*[contains(@class, ' map/map ')]/@booktitlealt)">
        <xsl:value-of select="//*[contains(@class, ' map/map ')]/@booktitlealt"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of>
          <xsl:apply-templates select="descendant::*[contains(@class, ' topic/topic ')][1]/*[contains(@class, ' topic/booktitlealt ')]" mode="dita-ot:text-only"/>
        </xsl:value-of>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
...
<!-- #AND commented out to stop creation of front pages 
