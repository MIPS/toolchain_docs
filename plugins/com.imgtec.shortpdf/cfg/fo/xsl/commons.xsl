            ...
            <fo:flow flow-name="xsl-region-body">
                <fo:block xsl:use-attribute-sets="topic">
                    ...
                    <xsl:choose>
                      ...
                      <!-- #AND comment out call to create mini-toc 
		      <xsl:otherwise>
...
-->
    ...
    <xsl:template match="*" mode="processTopicTitle">
	    ...
	    <!-- #AND
		Adding the block of code here to insert the numbering
	    -->
...
<!-- #AND 
	...
	Turned off heading numbering for the short form doc
	

-->
<!-- #AND start -->
	<xsl:variable name="enumber-levels" select="(false(), false(), false(), false())" />
	...
	<fo:inline><xsl:for-each select="$mapTopics[1]">
	...
	<xsl:number count="*[contains(@class, ' map/topicref ')]&#xA;  [not(ancestor-or-self::*[contains(@class, ' bookmap/frontmatter ')])]" format="1 " level="multiple" />
	</xsl:when>
	</xsl:choose>
	</xsl:for-each>
	</fo:inline>
...
<!-- Changed order of the title and figure so title is before figure-->
    <xsl:template match="*[contains(@class,' topic/fig ')]">
        <fo:block xsl:use-attribute-sets="fig">
            ...
            <xsl:if test="not(@id)">
             ...
             <xsl:apply-templates select="*[contains(@class,' topic/title ')]"/><xsl:apply-templates select="*[not(contains(@class,' topic/title '))]"/> 
