...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
	...
	<!-- #AND amending spacing 
	altered keep together to avoid issues with running off end of page 
	Set font size to 8pt
	#AND 30082017 added outputclass wideblock for very long code strings
	-->
	...
	<xsl:choose>
			<xsl:when test="@outputclass='wideblock'">6pt</xsl:when>	      
				<xsl:otherwise>8pt</xsl:otherwise>
		</xsl:choose>
	</xsl:attribute>
	<xsl:attribute name="start-indent">
		<xsl:choose>
			<xsl:when test="@outputclass='wideblock'">2mm</xsl:when>	      
				<xsl:otherwise>6pt + from-parent(start-indent)</xsl:otherwise>
		</xsl:choose>
	</xsl:attribute>
    </xsl:attribute-set>
