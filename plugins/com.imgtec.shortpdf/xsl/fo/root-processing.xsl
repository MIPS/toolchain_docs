...
<!-- #AND #revisionNumber 
    ...
    <xsl:variable name="productName">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:call-template name="insertVariable">
                    <xsl:with-param name="theVariableID" select="'Revision Number'"/>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:variable>
	...
	<xsl:variable name="revisionNumber"
	select="(//*opentopic:map//*[contains(@class,' topic/version ')]/@revision)[1]"/> 
	
	    <xsl:variable name="revisionNumber">
        <xsl:variable name="revisionNumber" select="(//*opentopic:map//*[contains(@class,' map/version ')]/@revision)[1]"/>
        <xsl:choose>
            <xsl:when test="$revisionNumber">
	...
	<xsl:variable name="revisionNumber">
		<xsl:choose>
		    <xsl:when test="$map//*[contains(@class, ' bookmap/revision ')][1]">
			<xsl:value-of>
				<xsl:apply-templates select="$map//*[contains(@class, ' bookmap/revision ')][1]" mode="dita-ot:text-only"/>
			</xsl:value-of>
		    </xsl:when>
		    <xsl:when test="$map//*[contains(@class, ' topic/revision ')][1]">
			<xsl:value-of>
				<xsl:apply-templates select="$map//*[contains(@class, ' topic/revision ')][1]" mode="dita-ot:text-only"/>
			</xsl:value-of>
		    </xsl:when>
		    <xsl:otherwise>
			<xsl:call-template name="insertVariable">
  ...
  <xsl:template match="/" mode="dita-ot:subject-metadata" as="xs:string?">
    ...
    <xsl:template match="/" name="rootTemplate">
        ...
        <fo:root xsl:use-attribute-sets="__fo__root">
           ...
           <!-- #AND stop creating front matter <xsl:call-template name="createFrontMatter"/> -->
