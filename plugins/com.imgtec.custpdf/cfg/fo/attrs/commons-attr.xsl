...
<!-- #AND 06022018 alter all boder widths to 1.25 -->
  <xsl:attribute-set name="common.border__top">
    <xsl:attribute name="border-before-style">solid</xsl:attribute>
    <xsl:attribute name="border-before-width">1.25pt</xsl:attribute>
  ...
  <xsl:attribute-set name="common.border__right">
    ...
    <xsl:attribute name="border-end-width">1.25pt</xsl:attribute>
    <xsl:attribute name="border-end-color">black</xsl:attribute>
  </xsl:attribute-set>

  <xsl:attribute-set name="common.border__left">
    <xsl:attribute name="border-start-style">solid</xsl:attribute>
    <xsl:attribute name="border-start-width">1.25pt</xsl:attribute>
    <xsl:attribute name="border-start-color">black</xsl:attribute>
  </xsl:attribute-set>
  
  <!-- #AND IMG border attribute sets -->
	<xsl:attribute-set name="img.border__left">
    <xsl:attribute name="border-start-style">solid</xsl:attribute>
    <xsl:attribute name="border-start-width">1.25pt</xsl:attribute>
    <xsl:attribute name="border-start-color">#006CB5</xsl:attribute>
  </xsl:attribute-set>
...
<!-- #AND set base font to sans serif -->
  ...
  <!-- titles 
	#AND 31012017 set title font to use titlesans Arial -->
  <xsl:attribute-set name="common.title">
    <xsl:attribute name="font-family">TitleSans</xsl:attribute>
  </xsl:attribute-set>

  <!-- #AND paragraph-like blocks 
	Reduced spacing to 0 to see effect on heading and other excess padding.-->
...
<!-- #AND This heading level is used for the heading at the mini-toc and the first level of heading -->
	...
	<!-- #AND temp take out the page break 
	<xsl:attribute name="break-before">page</xsl:attribute>
	#AND 26102017 switch colour to black

-->
	<xsl:attribute name="color">black</xsl:attribute>
	<xsl:attribute name="text-align">left</xsl:attribute>
	<xsl:attribute name="border-after-style">solid</xsl:attribute>
	<xsl:attribute name="border-after-color">#006CB5</xsl:attribute>
	...
	<!-- #AND  equiv to heading 1
	This level is used for main headings after the ToC 
		Added purple underline 20/11 as per branding pdf 
		Added page break before as per branding pdf -->
    <xsl:attribute-set name="topic.topic.title" use-attribute-sets="common.title">
        <xsl:attribute name="space-before">12pt</xsl:attribute>
        <xsl:attribute name="space-after">2pt</xsl:attribute>
	
	<!-- AND# 16022018 added outputclass pageBreak and removed default page break on x.x level headings. Page break can be forced by adding outputclass="pageBreak" to the title -->
	...
	<xsl:attribute name="keep-with-next.within-column">always</xsl:attribute>
	<!-- #AND remove underline as per branding meeting 12012017 <xsl:attribute name="border-after-style">solid</xsl:attribute>
	<xsl:attribute name="border-after-color">#72166B</xsl:attribute> -->
    </xsl:attribute-set>
    
	<!-- #AND removed border definitions -->
...
<!-- #AND equiv to heading 2
	20/11 Altered colour to black as per branding pdf 
	20/11 Added indent as per branding pdf 
	23/11 shouldn't have indent, -->
...
<!-- #AND 
	20/11 altered font-style to italic as per branding pdf. 
	030202017 added 5pt before -->
...
<!-- #AND 
	Section titles set to img purple, using common font and keep with next 
	Set indentation to be same as other indented headings.
-->
    <xsl:attribute-set name="section.title" use-attribute-sets="common.title">
        <xsl:attribute name="font-weight">bold</xsl:attribute>
        <xsl:attribute name="space-before">15pt</xsl:attribute>
	<!--#AND 1206 comment out indentation - causing issues on first page of chapters
	<xsl:attribute name="start-indent"><xsl:value-of select="$side-col-width"/></xsl:attribute> -->
	<xsl:attribute name="color">#006CB5</xsl:attribute>
        <xsl:attribute name="keep-with-next.within-column">always</xsl:attribute>
    </xsl:attribute-set>
...
<!-- #AND 
	Alignment of images is set in commons.xsl
	Increasing space top and bottom and adding a border
	06102017 removing border
	#AND 30102017 added keep-together.within-page">always to keep fig title with the figure. This has side-effect of pulling preceeding paragraph with the figure title onto next page in some circumstances.
-->
	...
	<!-- #AND 06102017 commented out border
	<xsl:attribute name="border">solid 1pt #6687B7 </xsl:attribute>  -->
	
    </xsl:attribute-set>

<!-- #AND 06102017  changed keep-with-previous to keep-with-next -->
...
<!-- #AND removed common.border then add zero width attribute to override pdf2 setting
	Then reverted back to using the border as good arguement put forward for this by AT.
-->
...
<!-- #AND 16012017 remove space on request of Ozana 
22082017 altered size to 100% to inherit size from cell -->
    ...
    <!-- #AND set the base font here to sans-serif  -->
        <xsl:attribute name="font-family">sans-serif</xsl:attribute>
...
<!-- #AND 01022018
	altered first value-of select from even to auto
-->
