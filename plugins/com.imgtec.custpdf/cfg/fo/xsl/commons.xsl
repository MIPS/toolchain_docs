        ...
        <fo:page-sequence master-reference="body-sequence" xsl:use-attribute-sets="page-sequence.body">
            ...
            <fo:flow flow-name="xsl-region-body">
                <fo:block xsl:use-attribute-sets="topic">
                 ...
                 <!-- #AND don't generate minitoc     
                          <xsl:apply-templates select="." mode="createMiniToc"/>
                       -->
		      <xsl:otherwise>
		      <!-- #AND start the code that inserts the first topic in the cell next to the minitoc -->
            ...
            <fo:flow flow-name="xsl-region-body">
                <fo:block xsl:use-attribute-sets="topic">
                    ...
                    <xsl:choose>
                     ...
                     <!-- #AND don't generate minitoc      <xsl:otherwise>
                          <xsl:apply-templates select="." mode="createMiniToc"/>
                      </xsl:otherwise> -->
		       <xsl:otherwise>
		      <!-- #AND start the code that inserts the first topic in the cell next to the minitoc -->
        ...
        <fo:block>
            ...
            <xsl:choose>
                <xsl:when test="$type = 'chapter'">
                    <!-- #AND comment out number selection <fo:block xsl:use-attribute-sets="__chapter__frontmatter__name__container">
                ...
                <xsl:when test="$type = 'appendix'">
                        <!-- #AND 1206 remove block for appendix since we just want the heading-->
...
<!-- #AND this controls numbering for ordinary topic.title
	I want numbering just on heading 1.
	Original text below
...
-->
    ...
    <xsl:template match="*" mode="processTopicTitle">
	    ...
	    <!-- #AND
		Try adding the block of code here to insert the numbering
	    -->
...
<!-- #AND 
	Adding variable 'level' for use in heading numbers 
	heading numbering

	Note that enumber-levels controls the levels displayed - currently set to one (only one 'true()').
	#AND 05102017 altering to 3 levels of number
-->
<!-- #AND start 
			...
			<xsl:for-each select="$mapTopics[1]">
					...
					<xsl:choose>
						<xsl:when test="parent::opentopic:map and contains(@class, ' bookmap/bookmap ')" />
							...
							<xsl:when test="ancestor-or-self::*[contains(@class, ' bookmap/appendix ')] and $enumber-levels[$depth]">
							<xsl:number count="*[contains(@class, ' map/topicref ')] [ancestor-or-self::*[contains(@class, ' bookmap/appendix ')]] " format="A&#8239;&#8239; " level="multiple" />
						</xsl:when>
						<xsl:when test="$enumber-levels[$depth]">
							<xsl:number count="*[contains(@class, ' map/topicref ')]  [not(ancestor-or-self::*[contains(@class, ' bookmap/frontmatter ')])]" format="1&#8239;&#8239; " level="multiple" />
						</xsl:when>
					</xsl:choose>
			</xsl:for-each>
		</fo:inline>
   ...
   <!-- #AND The following is the version from github -->
   
    <xsl:param name="numbers-before-title" select="'yes'"/>
        ...
        <fo:inline>
            <xsl:for-each select="$mapTopics[1]">
                ...
                <xsl:choose>
                    ...
                    <!-- Appendix topics -->
                    <xsl:when test="ancestor-or-self::*[contains(@class, ' bookmap/appendix ')] and $depth lt ($numbers-before-title-depth + 1)">
			...
			<xsl:number
                            count="*[contains(@class, ' topic/title ')][not(ancestor-or-self::*[contains(@class, ' bookmap/frontmatter ')])]"
    ...
    <!-- #AND github version insertion end -->
...
-->
  ...
  <xsl:template match="*[contains(@class, ' topic/div ')]">
    ...
    <xsl:template match="*[contains(@class,' topic/note ')]" mode="setNoteImagePath">
      ...
      <!-- #AND Commented out the below call to remove the pointing finger -->
      <!-- <xsl:call-template name="getVariable">
    ...
    <xsl:template match="*[contains(@class,' topic/fig ')]">
	    ...
	    <!--#AND 06102017  switched order  to put titles before figure image -->
	    <!--Original two lines 
...
<!-- #AND Amended so that if no alignments specified, default is to center image, otherwise fallback to standard default,
