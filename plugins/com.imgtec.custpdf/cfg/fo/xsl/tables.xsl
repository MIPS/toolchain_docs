<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  version="2.0">
    ...
    <!--  #AND Moved to lists.xsl Definition list
...
<!-- #AND replaced below with id select. May not work, might have to migrate my table customization into a copy of tables.xsl from pdf2
...
<!-- #AND select new attribute set if table uses simple_register outputclass --> 
<xsl:template match="*[contains(@class, ' topic/thead ')]/*[contains(@class, ' topic/row ')]">
        <xsl:choose>
		<xsl:when test="ancestor-or-self::*[contains(@class, ' topic/table ')][@outputclass='simple_register']">
		<fo:table-row xsl:use-attribute-sets="thead-simple-register.row">
			<!-- #AND  copy out border-bottom-style="solid" border-bottom-color="black" border-bottom-width="1pt" border-after-style="solid" border-after-color="black" border-after-width="1pt"-->
...
<!-- #AND Changing this so it always calls the generateTableEntryBorderHeader instead of generateTableEntryBorder. 
		generateTableEntryBorderHeader has a 'choose' that selects on class simple-register and sets colspan to 0 if so. -->
    <xsl:template match="*[contains(@class, ' topic/thead ')]/*[contains(@class, ' topic/row ')]/*[contains(@class, ' topic/entry ')]">
        <fo:table-cell xsl:use-attribute-sets="thead.row.entry">
            ...
            <xsl:call-template name="applyAlignAttrs"/>
            <xsl:call-template name="generateTableEntryBorderHeader"/>
	    <xsl:choose>
	    <!-- #AND 0806 inserted block to rotate text-->
	<xsl:when test="ancestor-or-self::*[contains(@class, ' topic/entry ')][@outputclass='rotate']">
		<fo:block-container reference-orientation="90" inline-progression-dimension="2cm">
    ...
    <xsl:template match="*[contains(@class, ' topic/tbody ')]/*[contains(@class, ' topic/row ')]/*[contains(@class, ' topic/entry ')]">
     	<fo:table-cell xsl:use-attribute-sets="tbody.row.entry">
		    ...
		    <!-- #AND 01022018 inserted block to rotate text start-->
		    <xsl:when test="ancestor::*[contains(@class, ' topic/table ')][1] and @outputclass='rotate' ">
			<fo:block-container reference-orientation="270" inline-progression-dimension="2cm">
				<fo:block xsl:use-attribute-sets="tbody.row.entry__content">
					<!-- <xsl:call-template name="processEntryContent"/> -->
				</fo:block>
			</fo:block-container>
		</xsl:when>
		<!-- #AND 01022018 inserted block to rotate text end-->
		    <xsl:otherwise>
...
<!-- #AND special table border generation for headers without colsep 
		colsep needs to be set to 0
-->
<xsl:template name="generateTableEntryBorderHeader">
