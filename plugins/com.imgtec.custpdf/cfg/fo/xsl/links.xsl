...
<!-- #AND 04082017 comment out call to insert the short description for relationship tables -->     
...
<!-- #AND updated with version from 2.4.4 -->
	...
	<!-- #AND add choose statement to turn off insertion of page number if noPageNo 
	outputclass is  on xref <xsl:when test="@outputclass='noPageNo'"> </xsl:when>
	-->
	
			
      <xsl:choose><xsl:when test="@outputclass='noPageNo'"> </xsl:when>
      <xsl:otherwise>
  ...
  <!-- Deprecated since 2.3   <xsl:template name="brokenLinks">
    <xsl:param name="href"/>
